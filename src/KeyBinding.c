...
void upArrow() {
  ...
  if (row > 0 && row - 1 < e.rowsNum &&
      (e.cx + e.coloff >= e.rowBuff[row].len ||
       e.cx + e.coloff > e.rowBuff[row - 1].len))
...
void downArrow() {
  ...
  if (row + 1 < e.rowsNum && (e.cx + e.coloff >= e.rowBuff[row].len ||
                              e.cx + e.coloff > e.rowBuff[row + 1].len))
    moveCursorToIndex(e.rowBuff[row + 1].len);

  if (e.cy != e.windowsLength - 1)
    e.cy++;
  else
    e.rowoff++;
}
void rightArrow() {
  if (e.cy + e.rowoff < e.rowsNum &&
      e.cx + e.coloff == e.rowBuff[e.cy + e.rowoff].len) {
    // if (e.cc != e.windowsLength -1) e.cy++;
    // else e.rowoff++;
    // e.cx=0;
    // e.coloff=0;
  } else if (e.cx != e.windowsWidth - 1)
    e.cx++;
  else
    e.coloff++;
}
...
void gotoNextWord() {
  ...
  if (nextWord == -1) {
    if (e.cy < e.windowsLength - 1 && e.cy < e.rowsNum - 1)
      e.cy++;
...
void gotoPrevWord() {
  ...
  if (prevWord == -1) {
    ...
    int prev =
        prevSpace(e.rowBuff[e.cy + e.rowoff].b, e.rowBuff[e.cy + e.rowoff].len);
    if (prev == -1)
      e.cx = 0;
    else
