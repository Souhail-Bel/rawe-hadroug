  ...
  case SAVE: {
    ...
    int messageLen =
        snprintf(message, sizeof(message),
                 (e.modification_num > 1) ? "%d modifications written to disk !"
                                          : "%d modification written to disk !",
                 e.modification_num);
  ...
  while (1) {
    ...
    case ESCAPE: {
      ...
      case '[':
        switch (seq[1]) {
        case RIGHT_ARROW:
          if (command.len && e.cx != command.len - 1)
            e.cx++;
          break;
        case LEFT_ARROW:
          if (e.cx)
    ...
    case BACKSPACE2:
      if (e.cx != promptLen) {
        ...
        int charLen = utf8_len(
            command.b[getPosInBytes(e.cx - 1, command.b, command.lenByte)]);
        removeCharInRow(&command, posInBytes, charLen);
        e.cx--;
      }
      break;
    default: {
